...
const TableUI: React.FC = () => {
    const {columnsConfig , data , formatterConfig , formatterHandler} = useGetDashboardContext();
                            ...
                            cellRenderer: (params: any) => {
                                return <div onClick={()=>{
                                    if(formatterHandler){
                                        ...
                                        params.data.isFavorite = params.data.isFavorite ? false : true;
                                        params.api.refreshCells({ rowNodes: [params.node], force: true });
                                    }
                                    
                                }} className="flex justify-center items-center">
                                   <p className="m-0">
                                        <span className="text-[#dbb42c] text-lg"> { !params.data.isFavorite ? '☆' : '★'}</span>
                                    </p>
                                </div>
                            },
                            width: 100,
                    ...
                    case "profitFormatter":{
                        cols = [ ...cols , {
                            ...
                            cellRenderer: (params: any) => {
                                ...
                                const percent = revenu > 0 ? Math.round((profit / revenu) * 100) : 0;
                                return <div>
                                    <Progress percent={revenu} success={{ percent : profit}} status="active"/>
                                </div>
                            },
                        }];
                        break;
    ...
    return (
        <div className="ag-theme-alpine" style={{ height: 400, width: '100%' }}>
            <AgGridReact rowData={data} columnDefs={columns} gridOptions={{theme: 'legacy'}} pagination={true} paginationPageSize={10} paginationPageSizeSelector={[5,10,20]} />
        </div>
    )
};
