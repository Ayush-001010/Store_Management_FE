...
const Layout: React.FC<ILayout> = ({ chat, applySendHandler }) => {
    ...
    <div className="flex justify-end">
      <div className="w-1/2 bg-[#f8f9fa] text-sm text-[#495057] p-2 font-medium rounded-lg shadow-md">
        ...
        <div>
            ...
            {AICreateLayoutConfig.sectionDetailsConfig.map((section, index) => (
                <div key={index} className="mt-4 p-2 bg-[#e9ecef] rounded-lg flex flex-col items-start justify-start">
                    <h3 className="font-semibold text-[#212529] mb-1 cursor-pointer hover:underline" onClick={()=>applySendHandler(`Let's go with ${section.title} Section`,"Title")}>{section.title}</h3>
