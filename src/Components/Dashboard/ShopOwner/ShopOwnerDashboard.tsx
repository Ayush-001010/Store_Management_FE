...
const ShopOwnerDashboard: React.FC<IShopOwnerDashboard> = () => {
  const {
    ...
    cardValues,
    analyticValue,
    getDataAccordingToHeaderOption,
    setIsFavorite,
    analyticData,
    applyHandlerOfAnalytic,
    storeOpt,
    analyticPoints,
    storeData,
  } = useShopOwnerDashboardAction();
  const { userName } = useSelector(
    (state: any) => state.userDetails as UserDetailsType
  );
  const [analyticOptions, setAnalyticOptions] = useState(
    ShopOwnerDashboardConfig.analyticBarOptions
  );
  const [headerOptionsValue, setHeaderOptionsValue] = useState<string | null>(
    null
  );
  ...
  const analyticBarChangeHandler = (value: string) => {
    ...
    if (storeId) {
      setAnalyticOptions((prev) => {
        ...
        if (
          prev.length === ShopOwnerDashboardConfig.analyticBarOptions.length
        ) {
          return [...prev, obj];
        }
        return [...prev];
  ...
  const favoriteFormatterHandler = async (name: string, value: any) => {
    console.log("Favorite Formatter Handler Called: ", name, value);
    switch (name) {
      case "FavoriteFormatter": {
        await setIsFavorite(value.id, value.isFavorite ? false : true);
        break;
      }
    }
  };
  const headerOptionHandler = async (name: string) => {
    console.log("Header Option Handler Called: ", name);
    if (name !== headerOptionsValue) {
      ...
      await getDataAccordingToHeaderOption("All_Store");
    }
  };
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex mt-4 w-full">
        <AnalyticPoints
          analyticBarChangeHandler={analyticBarChangeHandler}
          data={analyticPoints}
        />
        <AnalyticBar
          specificFieldSelectionHandler={specificFieldSelectionHandler}
          isSpecificFieldSelected={true}
          specificFieldName="range"
          applyHandler={applyHandlerOfAnalytic}
          chartType={analyticValue.chartType === "lineChart" ? "line" : "bar"}
      ...
      <div className="m-4 p-3 bg-[#f8f9fa] shadow-md rounded-lg">
        <Dashboard
          headerOptionsValue={headerOptionsValue}
          headerOptionHandler={headerOptionHandler}
          columnsConfig={ShopOwnerDashboardConfig.storeColumnConfig}
          data={storeData}
